...
export const getColorObjects = () => {
  ...
  for (let i = 0; i < twColorList.length; i++) {
    ...
    for (let v = 100; v < 1000; v = v + 100) {
      let newname = `${color}-${v}`;
      let obj = new ColorObj(color, 50, twColors[color][v]);
      array.push(ob);
    }
    result.push(array);
  }
...
const DEFAULTCOLOROBJ = new ColorObj("teal", 500, twColors.teal[500]);
...
export const makeCustomShades = (hex, catname) => {
  ...
  if (colorname !== "custom") {
    ...
    keys.forEach((key) => {
      let obj = new ColorObj(colorname, key, twColors[colorname][key]);
    ...
    keys.forEach((key) => {
      ...
      let obj = new ColorObj(catname, key, shadehex);
