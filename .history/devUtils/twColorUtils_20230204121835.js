...
import { hexToHSL, HSLToHex } from "./colorLib";
...
export const getColorObjByHex = (hex, category, useHexOnly = false) => {
  ...
  if (typeof tw === "object" && useHexOnly == false) {
    ...
    return new ColorObj(category, "", hex);
  } else {
    return new ColorObj("custom", "", hex);
  }
};
...
export const initVariants_old = (passedtheme) => {
...
export const initVariants = (passedtheme) => {
  ...
  for (const idx in myColors) {
    ...
    variants = updateVariants(category, category, newtheme);
...
export const updateVariants_old = (base, category, passedtheme) => {
  //teal, brand, theme
