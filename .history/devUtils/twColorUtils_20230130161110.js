...
export const makeCustomShades = (hex, catname) => {
  ...
  if (twColorList.includes(colorname)) {
    //this is a tailwind color
    let twKeys = Object.keys(twColors[colorname]);
    console.log("twcolors:", twColors);
    customShades.push(DEFAULTCOLOROBJ);
  } else {
...
export const getTWColornameByHex = (hex) => {
  for (let colorname in twColors) {
    if (typeof twColors[colorname] == "object") {
       if (hex == twColors[colorname].DEFAULT); { return colorname;  }
    } else {
    return "custom";
   }
};
...
export const initVariants = (passedtheme) => {
  ...
  for (const idx in myColors) {
    ...
    if (!newtheme.variants.hasOwnProperty(colorname)) {
      newtheme.variants[colorname] = {};
    }
    newtheme.variants[colorname] = variants;
  }
