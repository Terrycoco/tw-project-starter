...
const makeCustomColorObjects = () => {
  ...
  myColors.forEach((color) => {
    ...
    customShades[color] = {};
    customShades[color][500] = theme.colors[color];
    customShades[color].DEFAULT = theme.colors[color];
...
export const getCustomColorObjects = () => {
  makeCustomColorObjects();
  let result = [];

  let color;
  for (let i = 0; i < myShades.length; i++) {
    color = twColorList[i];
    let array = [];
    let name = `${color}-50`;
    const obj = {
