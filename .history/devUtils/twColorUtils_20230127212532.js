...
const makeCustomShades = () => {
  //make color object as whole
  myColors.forEach((colorname) => {
    let hex = theme.colors[colorname]; //get hex
    customShades[colorname] = {}; //add color to obj
    customShades[color][500] = theme.colors[colorname]; //defaults
    customShades[color].DEFAULT = theme.colors[colorname]; //default
    //for each val in shadeVals
    let keys = Object.keys(shadeVals);
    keys.forEach((key, idx) => {
      customShades[color][key] = lightenDarkenColor(hex, shadeVals[key]);
    });
  });
};

export const getCustomColorObjects = () => {
  makeCustomShades();
  let result = [];
  ...
  myColors.forEach((color) => {
    ...
    for (let v = 100; v < 1000; v = v + 100) {
      let newname = `${color}-${v}`;
      let ob = makeColorObj(newname, color, v, customShades[color][v]);
      array.push(ob);
    }
    result.push(array);
  });
