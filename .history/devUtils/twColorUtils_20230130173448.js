...
class ColorObj {
  constructor(base, variant, hex) {
    this.id = base + ( variant ? "-" + variant : null);
    this.base = base;
...
const makeColorObj = (id, category, colorname, value, hex) => {
  return {
    new ColorObj(id, colorname, value, hex)
    id: id, //brand-500  teal-500
    colorname: colorname, //teal
    category: category, //brand
    value: value, //500 //DEFAULT
    hex: hex,
...
export const getColorObjects = () => {
  ...
  for (let i = 0; i < twColorList.length; i++) {
    ...
    for (let v = 100; v < 1000; v = v + 100) {
      let newname = `${color}-${v}`;
      let ob = makeColorObj(newname, null, color, v, twColors[color][v]);
      array.push(ob);
    }
    result.push(array);
  }
...
const DEFAULTCOLOROBJ = {
  ...
  category: null,
  value: 50,
  hex: twColors.teal[50],
};

//make each time because may change
export const makeCustomShades = (hex, catname) => {
  console.log("hex", hex, "parent", catname);
  let customShades = {};
  if (!isValidHexCode(hex)) return "ERROR";
  customShades = {}; //add color to obj
  customShades.DEFAULT = hex; //default

  //if this is a tw color just get normal variants
  //lookup colorname
  let colorname = getTWColornameByHex(hex);
  if (colorname !== "custom") {
    //this is a tailwind color
    let keys = Object.keys(twColors[colorname]);
    keys.forEach((key) => {
      let obj = makeColorObj(
        `${colorname}-${key}`,
        catname, //cat
        colorname, //colorname
        key, //value
        twColors[colorname][key] //hex
      );
...
export const getCustomColorObjectsArray = (hex, colorname) => {
  if (!hex) return;
  let customShades = makeCustomShades(hex, colorname);
  if (customShades === "ERROR") return;
  let array = [];
  let name = `${colorname}-50`;
  let obj = makeColorObj(name, colorname, 50, customShades[50]);
  array.push(obj);

  //inner loop
  for (let v = 100; v < 1000; v = v + 100) {
    let newname = `${colorname}-${v}`;
    let ob = makeColorObj(newname, colorname, v, customShades[v]);
    array.push(ob);
  }
...
export const getTWColornameByHex = (hex) => {
  for (let colorname in twColors) {
    if (typeof twColors[colorname] == "object") {
      if (hex == twColors[colorname].DEFAULT);
      {
        return colorname;
      }
    } else {
  ...
  for (let color in twColors) {
    if (typeof twColors[color] == "object") {
      ...
      for (let colorVal in twColors[color]) {
        ...
        result[hex] = makeColorObj(name, null, color, colorVal, hex);
      }
    } else {
      hex = twColors[color];
...
export const replaceColorWithHex = (classStr = "") => {
  //default
  let result = classStr;
  ...
  colorCategories.forEach((cat) => {
    ...
    if (result.includes(`bg-${colorClass}`)) {
      let bgcl = result.replace(/`bg-${colorClass}`/g, `bg-[${colorHex}]`);
      result = ` ${bgcl}`;
    }
    if (result.includes(`text-${colorClass}`)) {
      let txtcl = result.replace(/`text-${colorClass}`/g, `text-[${colorHex}]`);
      result = ` ${txtcl}`;
    }
  });
...
export const initVariants = (passedtheme) => {
  ...
  for (const idx in myColors) {
    let catname = myColors[idx];
    let hex = passedtheme.colors[catname];
    let variants = makeCustomShades(hex, catname); //# brand
    //console.log("returned from make shades", variants);

    if (!newtheme.hasOwnProperty("variants")) {
      newtheme.variants = {};
    }
    if (!newtheme.variants.hasOwnProperty(catname)) {
      newtheme.variants[catname] = {};
    }
    newtheme.variants[catname] = variants;
  }
