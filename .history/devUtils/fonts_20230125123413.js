...
const getFontsByCategory = () => {
  ...
  fontKeys.forEach((font) => {
    for (const cat in fonts[font].category) {
      console.log("category:", font[font].category);
      let category = fonts[font].category[cat];
      result[category].push(fonts[font].name);
    }
  });
