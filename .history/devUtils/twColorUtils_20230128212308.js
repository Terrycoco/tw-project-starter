...
export const initVariants = (currentTheme) => {
  let variants = makeCustomShades();
  const myColors = Object.keys(currentTheme.colors);
  for (const idx in myColors) {
    let colorname = myColors[idx];
    if (!theme.hasOwnProperty("variants")) {
      currentTheme.variants = {};
    }
    if (!theme.variants.hasOwnProperty(colorname)) {
      currentTheme.variants[colorname] = {};
    }
    currentTheme.variants[colorname] = variants;
  }
  return currentTheme;
};
