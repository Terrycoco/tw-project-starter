...
export const getPctOfHex = (hex, pct) => {
  //how many units we can lighten
  const white = "#ffffff";
  let notWhite = "#ffffff";
  let incr = 226;
  do {
    incr -= 1;
    notWhite = lightenDarkenColor(hex, incr);
  } while (notWhite === white);
  console.log("incr is ", incr);

  //take resulting incr and divide by 100
  let onepct = incr / 100;
  console.log("onepct:", onepct);
  //pct of incoming means opacity so reverse it
  let desired = Math.floor(onepct * (100 - pct));

  //return desired lightness
  let result = lightenDarkenColor(hex, desired);
