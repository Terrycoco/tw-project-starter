...
export const makeCustomShades = (hex, colorname, passedTheme) => {
  let customShades = {};
  if (!isValidHexCode(hex)) return "ERROR";
  customShades = {}; //add color to obj
  customShades.DEFAULT = passedTheme.colors[colorname]; //default
  //for each val in shadeVals
  let keys = Object.keys(shadeVals);
  keys.forEach((key, idx) => {
    customShades[key] = lightenDarkenColor(hex, shadeVals[key]);
  });
...
export const initVariants = (passedtheme) => {
  ...
  for (const idx in myColors) {
    ...
    let variants = makeCustomShades(hex, colorname, passedtheme);
