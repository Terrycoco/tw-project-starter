...
const getConfigColors = (theme) => {
  ...
  for (const colorname in theme.colors) {
    res[colorname] = {};
    res[colorname]["DEFAULT"] = theme.colors[colorname];
    for (const key in theme.variants[colorname]) {
      res[colorname][key] = theme.variants[colorname][key].hex;
    }
  }
  return res;
};
...
let myColors = getConfigColors(initializedTheme);
  ...
  theme: {
    ...
    extend: {
      colors: {
        ...
        blackish: { ...myColors.blackish }, //not quite black
        whitish: { ...myColors.whitish }, //not quite white
      },
    }, //end extend
  }, //end theme
