...
const HexInput = (props) => {
  ...
  return isValid === true ? (
    <div id={`hex${props.id}`} className="w-full mb-3"><Draggable id="hexinput">{renderForm()}</Draggable> <button
  ...
  >
    ...
    <div id={`hex${props.id}`} className="w-full mb-3">{renderForm()}<button
    className={theme.buttons.sm}
    type="button"
    onClick={handleClear}
  >
