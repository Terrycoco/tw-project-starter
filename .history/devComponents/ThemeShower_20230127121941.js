...
const ThemeShower = (props) => {
  ...
  const loopThroughTheme = (themeObj, isChild = false, output = "") => {
    //for each key in this object
    Object.keys(themeObj).map((key) => {
      //if child is another object loop again
      if (typeof themeObj[key] === "object") {
        output += `${key}: {\n\r`;
        output += loopThroughTheme(themeObj[key], true, output);
        output += `},\n\r`;
      } else {
        //this has an end value
        let indent = isChild ? `/t` : null;
        output += `${isChild} ${key}: "${themeObj[key]}",/n/r`;
      }
    });
    return output;
  };
  ...
  const getThemeToCopy = () => {
    const themeObj = theme;
    return loopThroughTheme(themeObj); //no optional lineendings
  };
