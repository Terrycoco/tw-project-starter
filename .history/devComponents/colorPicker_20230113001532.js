...
const ColorBlock = ({ color, selectedColor, ...props }) => {
  ...
  useEffect(() => {
    if (selectedColor === color) {
      setCl(`text-center ${color} border-4 border-blue-500`);
    } else {
  ...
  return (
    <div className={cl}>
      <div id={color} className={cl} onClick={handleClick}></div>
    </div>
  );
};
...
const ColorPicker = ({ visible, ...props }) => {
  ...
  useEffect(() => {
    if (visible === true) {
      setCl(
        "p-3 w-1/2 absolute top-0 right-0 bg-white border-1 border-gray-500 shadow-left"
      );
    } else {
      setCl("hidden");
    }
