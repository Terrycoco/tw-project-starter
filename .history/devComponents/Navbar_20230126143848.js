...
const Navbar = (props) => {
  ...
  useEffect(() => {
    ...
    cl = twMerge(outer, props.className);
    setOuter(cl);

    // //define function
    // const forceColor = (latestClass) => {
    //   //for dev only -- get around non-safelisted colors
    //   let colorIdx = latestClass.indexOf("#");
    //   let hex = latestClass.substr(colorIdx, 7);
    //   console.log("hex is", hex);

    //   if (latestClass.substr(colorIdx - 2, 3) == "bg-") {
    //     console.log("is bg");
    //   }
    // };

    // forceColor(cl);
  }, [outer, props.className, theme.layout.header]);
  ...
  return (
    <>
      <div className={`sticky top-0 left-0 w-screen z-1000 `}>
        <nav id={props.id} className={outer}>
