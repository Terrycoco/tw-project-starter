  ...
  return (
    <div className={cl}>
      <FlexLayout className={"py-4 justify-between"}>
        ...
        {showAlert === true ? (
          <div
            className={`${showAlert} leading-tight flex flex-col justify-center items-center bg-green-500 text-whitish text-xs px-3 font-medium`}
          >
            Copied!
          </div>
        ) : null}
      ...
      <div className="font-code top-0 w-full z-50  min-h-screen bg-zinc-700 text-whitish pt-2 px-20">
        <pre className="whitespace-pre-wrap" key="preamble">
          {pre}
        </pre>
        <pre className="whitespace-pre-wrap" key="themetext">
          {getThemeText(props.theme)}
        </pre>
        <pre className="whitespace-pre-wrap" key="post">
          {post}
        </pre>
      </div>
    </div>
