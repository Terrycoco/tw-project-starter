...
const ColorBlock = ({ color, selectedColor, ...props }) => {
  const [cl, setCl] = useState(` ${color}`); //class of block
  const [hex, setHex] = useState(twColors[props.key]); //hex val of block
  const bg = `bg-${color}`; //color name
  console.log(cl);
  const handleClick = (e) => {
...
const ColorPicker = ({ visible, ...props }) => {
  ...
  useEffect(() => {
    if (visible === true) {
      setCl(
        "p-3 w-1/2 absolute visible top-0 right-0  white border-1 border-slate-900 shadow-left"
      );
    } else {
      setCl("hidden");
    }
  ...
  return (
    <div className={"bg-white w-1/2 absolute top-0 right-0"}>
