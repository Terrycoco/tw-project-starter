...
const ThemeShower = (props) => {
  ...
  const getThemeToRender = () => {
    loopThroughObjRecurs(theme, addLine);
  ...
  const addLine = (key, val) => {
    return `${key}: "${val}"`;
  };

  function loopThroughObjRecurs(obj, propExec, output = "") {
    for (var k in obj) {
      if (typeof obj[k] === "object" && obj[k] !== null) {
        output += `${k}: {\n\r`;
        output += loopThroughObjRecurs(obj[k], propExec, output);
      } else if (obj.hasOwnProperty(k)) {
        output += propExec(k, obj[k]);
      }
    }
   return output;
