...
const Stylesheet = (props) => {
  ...
  const getColorClass = (elementClass = "") => {
    //default
    let result = " " + `bg-whitish`;
    ...
    colorCategories.forEach((cat) => {
      ...
      if (elementClass.includes(colorClass)) {
        let newcl = elementClass.replace(`bg-${colorClass}`, `bg-[${hex}]`);
        let obj = getColorObjByHex(colorHex).hex;
        result = ` ${newcl} text-${obj.textColor}`;
        console.log("color returns: ", result);
      }
    });
  ...
  return (
    ...
    >
      <Navbar
        ...
        outerStyles={"shadow-none  text-whitish"}
      ...
      >
        <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          ...
          <FlexLayout className="mt-5 gap-5" key="topsection">
            <FlexColumn key="col1" className="md:w-2/3 pr-0 ">
          ...
          <FlexColumn key="textVals" className="w-full md:w-2/3 md:pr-12">
            <div>
              <Heading id="h1test" className={getClasses("text", "heading")}>
                Headings have a high contrast
              </Heading>
              <Subhead className={getClasses("text", "subhead")}>
                Subheadings - Smaller than headings{" "}
              </Subhead>
              <H3 className={getClasses("text", "h3")}>This is H3</H3>
