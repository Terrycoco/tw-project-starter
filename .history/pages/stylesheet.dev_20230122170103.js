...
const Stylesheet = (props) => {
  ...
  useEffect(() => {
    ...
    // } else {
    //   //storage is there let's load it
    //   setTheme(stored);
    // }
  }, []);

  const updateTheme = (newValue, name, cat) => {
    let newtheme = Object.assign({}, (theme[cat][name] = newValue), ...theme);
        ...
        <FlexLayout key="themeplay" className="gap-5">
          <FlexColumn key="textVals" className="w-full md:w-2/3">
            <div>
              <h1
                id="h1test"
                className={theme.text.h1}
                style={getFontStyle(theme.text.h1)}
              >
