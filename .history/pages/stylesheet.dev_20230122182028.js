...
const Stylesheet = (props) => {
  ...
  const getClasses = (cat, name) => {
    return theme[cat][name];
  };

  const getFontStyle = (elementClass) => {
    let fontClass = "";
    //find what font cat this is
    if (elementClass.includes("font-display")) {
      fontClass = getFont(theme.fonts.display);
      return {
        ...
        <FlexLayout key="themeplay" className="gap-5">
          <FlexColumn key="textVals" className="w-full md:w-2/3">
            <div>
              <h1
                id="h1test"
                className={getClasses("text", "h1")}
                style={getFontStyle(getClasses("text", "h1"))}
              >
