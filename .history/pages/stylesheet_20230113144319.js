...
const Stylesheet = (props) => {
  ...
  const themeForm = (parentKey, key, value) => {
    if (parentKey) {
      return (
            <div key={parentKey} className="kicker uppercase">
            {parentKey}
          </div>    
      )
    } else {
  ...
  const loopThroughTheme = (currState) => {
    Object.keys(currState).map((key) => {
      //only print out lower level children
      if (typeof themeState[key] === "object") {
        return (
          
        );
        Object.keys(themeState[key]).map(child => {
