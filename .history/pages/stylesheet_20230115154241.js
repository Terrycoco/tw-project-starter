...
const Stylesheet = (props) => {
  ...
  const toggleColorPicker = (e, forceClose) => {
    if (forceClose === undefined || forceClose !== true) {
      console.log("got here:", forceClose);
      ...
      <div className="relative w-full min-h-screen overflow-x-hidden overflow-y-hidden px-6">
        ...
        >
          <div id="colors-result" className={styles.result}>
            <h2 className={theme.h2}>Color Palette</h2>
            ...
            <div id="colorplay" className="flex flex-col space-y-2 py-8">
              <h3 className={theme.h3}>Play With Colors</h3>
              <div
                className={`h-10 w-1/2 min-h-full p-3 border border-black text-sm font-bold ${selectedColor} ${textColor}`}
              >
                {`${selectedColor}`}
              </div>
              <div
