...
const Stylesheet = (props) => {
  ...
  const themeForm = (key, value, isParent = false, cat) => {
    if (isParent === true) {
      ...
      if (cat === "colors") {
        ...
        let showVars = key.includes("whitish") ? false : true;
        return (
  ...
  const toggleColorPicker = (e, open) => {
    ...
    e.stopPropagation();
    if (open === true) {
      setShowTWPicker(true);
    } else {
      setShowTWPicker(false);
    }
  };
            ...
            >
              ...
              <div className="pt-3">
                <div className="flex flex-row justify-between align-bottom">
                  ...
                  >
                    gradient example (secondary to primary)
                  </label>
                </div>
                <div
                  className={`h-10 p-3 border border-blackish text-sm font-bold  text-blackish`}
                  style={{
                    backgroundImage: `linear-gradient(to right, ${theme.colors.secondary}, ${theme.colors.primary})`,
                  }}
                >
                  gradient-sec-to-prim
                </div>
              </div>
            </FlexColumn>
